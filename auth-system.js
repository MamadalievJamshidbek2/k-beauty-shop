...
(function() {
  const USERS_STORAGE_KEY = 'myshop_users_v1';
  ...
  function setCurrentUser(user) {
    if (user) {
      ...
      localStorage.setItem('currentUser', user.name); // For reviews
    } else {
  ...
  function login(email, password) {
    ...
    return { success: true, message: 'Xush kelibsiz!', user };
  }
  ...
  // Setup login modal (existing one in parfum.html)
  ...
  function showUserMenu(button) {
    ...
    menu.innerHTML = `
      <div style="padding: 15px; border-bottom: 1px solid #eee;">
        <div style="font-weight: 600; color: #111; margin-bottom: 5px;">${currentUser.name}</div>
        <div style="font-size: 13px; color: #666;">${currentUser.email}</div>
      </div>
      <a href="wishlist.html" style="display: block; padding: 12px 15px; color: #333; text-decoration: none; transition: background 0.3s;">
        ❤️ Sevimlilar
      </a>
  ...
  function updateUserInterface() {
    ...
    if (currentUser && openLoginBtn) {
      ...
      openLoginBtn.title = currentUser.name;
    }
  }
